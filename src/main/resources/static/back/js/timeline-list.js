layui.use(["laydate", "table", "util", "form"], function () {
    ...
    table.render({
        ...
        cols: [
            [
                ...
                {field: 'content', title: '更新内容', width: 230},
                {
                    field: 'pushDate', title: '更新时间', width: 230,
                    templet: function (d) {
                        return util.toDateString(d.pushDate, "yyyy-MM-dd");
                    }
                },
                {
    ...
    form.on('submit(search)', function (data) {
        table.reload("timelineTable", {
            url: "/timeline/search",
            where: {
                content: data.field.content,
                pushDate: data.field.pushDate
            },
            method: "POST",
            parseData: function (res) {
                return {
    ...
    // 监听操作栏点击事件
    table.on('tool(timeline)', function (obj) {
        let layEvent = obj.event;
        let data = obj.data;
        if ("edit" === layEvent) {
            window.parent.xadmin.add_tab(util.toDateString(data.pushDate, "yyyy-MM-dd"), "/admin/timeline-detail?timelineId=" + data.id + "&token=" + localStorage.getItem("token"))
