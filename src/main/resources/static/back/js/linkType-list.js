layui.use(["laydate", "table", "util", "form"], function () {
    ...
    table.render({
        ...
        height: 450,
        id: "linkTypeTable",
        url: "/link_type/listLinkTypePage",
        page: true,
        method: "get",
        toolbar: true,
        ...
        cols: [
            [
                ...
                {field: 'name', title: '链接类型名', width: 230},
                {
    ...
    form.on('submit(search)', function (data) {
        table.reload("linkTypeTable", {
            url: "/link_type/search",
            where: {
    ...
    // 监听操作栏点击事件
    table.on('tool(linkType)', function (obj) {
        let layEvent = obj.event;
        let data = obj.data;
        if ("edit" === layEvent) {
            window.parent.xadmin.add_tab(data.name, "/admin/linkType-detail?linkTypeId=" + data.id + "&token=" + localStorage.getItem("token"))
        } else if ("delete" === layEvent) {
            // 首先弹出询问窗口进行确认
            if (data.status === 0) {
                layer.msg("不能删除已经删除的项！", {icon: 2});
            } else {
                layer.confirm('该操作会删除该类型下所有连接, 确认删除？', {
                    btn: ['确认', '取消']
                }, function (index) {
                    $.ajax({
                        url: "/link_type/" + data.id,
                        method: "DELETE",
                        dataType: "json",
