layui.use(["laydate", "table", "util", "form"], function () {
    ...
    table.render({
        ...
        cols: [
            [
                ...
                {field: 'url', title: 'url', width: 230},
                {field: 'name', title: '链接名', width: 230},
                {field: 'type', title: '链接类型', width: 230},
                {
    ...
    table.on('tool(link)', function (obj) {
        ...
        let data = obj.data;
        if ("edit" === layEvent) {
            window.parent.xadmin.add_tab(data.name, "/admin/link-detail?linkId=" + data.id + "&token=" + localStorage.getItem("token"))
