<template>
  ...
  <div class="content-side pa">
    ...
    <div class="tags mb15">
      ...
      <a :href="'/articles/' + blogType.name" class="tag" v-for="blogType in blogTypeList" :key="blogType.id" v-show="blogType.status === 1">
        {{blogType.name}}
      </a>
    </div>
    <div class="links mb15">
      ...
      <a :href="link.url" class="link" v-show="link.status === 1" v-for="link in linkList" :key="link.id">
        {{link.name}}
      </a>
    </div>
  </div>
...
<script>
  ...
  import {listFriendLinkAll} from "../api/link";
  import {listGuessYouLike} from "../api/blog";
  ...
  export default {
    ...
    mounted() {
      ...
      this.listFriendLinkAll();
      this.listGuessYouLike();
    }
  }
</script>
