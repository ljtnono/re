...
public enum ReErrorEnum {
    ...
    /** 请求参数错误 */
    REQUEST_PARAM_ERROR(40001, "请求参数错误"),

    //**************************用户相关**************************//
    /** 用户名格式错误 */
    USERNAME_FORMAT_ERROR(40002, "用户名是4-16位字符（字母，数字，下划线，减号）"),
    /** 密码格式错误 */
    PASSWORD_FORMAT_ERROR(40003, "密码是6-20位字符，必须包括字母和数字与特殊字符（!@#$%^&*?）的其中一种"),
    /** 用户不存在 */
    USER_NOT_EXIST(40004, "用户不存在"),
    /** 用户名重复 */
    USERNAME_ALREADY_EXIST(40005, "用户名重复"),
    /** 验证码过期 */
    VERIFY_CODE_EXPIRED(40006, "验证码过期"),
    /** 验证码错误 */
    VERIFY_CODE_ERROR(40007, "验证码错误"),
    /** 邮箱格式错误 */
    EMAIL_FORMAT_ERROR(40008, "邮箱格式错误"),
    /** 手机号码格式错误 */
