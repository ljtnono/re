...
public class RoleService {
     ...
     */
    public void addRole(RoleDTO reRoleDTO) {
        ...
        reRole.setDeleted(StatusEnum.ENTITY_IS_DELETED_NOT_DELETED.getValue());
        setCreateTimeAndModifyTimeNow(reRole);
        int insert = roleMapper.insert(reRole);
        if (insert <= 0) {
        ...
        if (isPermissionExists) {
            // 插入角色权限表
            rolePermissionService.insertRolePermission(reRole.getId(), permissionIdList);
        } else {
            // 权限不存在
     ...
     *
     */
    public void updateRole(RoleDTO reRoleDTO) {
        Optional.ofNullable(reRoleDTO)
                .orElseThrow(() -> new ParamException(GlobalErrorEnum.REQUEST_PARAM_ERROR));
        // 检测该角色是否存在
        ...
        Role reRole = roleMapper.selectOne(new LambdaQueryWrapper<Role>()
                .select(Role::getName)
                .eq(Role::getName, reRoleDTO.getName())
                .ne(Role::getId, reRoleDTO.getId()));
        Optional.ofNullable(reRole)
                .orElseThrow(() -> new ResourceAlreadyExistException(GlobalErrorEnum.ROLE_ALREADY_EXIST));
        // 插入角色表
        int update = roleMapper.update(null, new LambdaUpdateWrapper<Role>()
                .set(Role::getName, reRoleDTO.getName())
                .set(Role::getModifyTime, new Date())
                .eq(Role::getId, reRoleDTO.getId()));
        if (update <= 0) {
            throw new DataBaseException(GlobalErrorEnum.DATABASE_OPERATION_ERROR);
        }
        // 插入角色权限表
        ...
        if (isPermissionExists) {
            // 先删除角色原来的权限
            rolePermissionService.deleteRolePermissionByRoleId(reRoleDTO.getId());
            // 插入新的角色权限关系
            rolePermissionService.insertRolePermission(reRoleDTO.getId(), permissionIdList);
        } else {
            // 权限不存在
            throw new ResourceNotExistException(GlobalErrorEnum.PERMISSION_NOT_EXIST);
        }
     ...
     */
    @Transactional(readOnly = true)
    public IPage<Role> getRoleListPage(RoleDTO reRoleDTO) {
        Optional.ofNullable(reRoleDTO)
                .orElseThrow(() -> new ParamException(GlobalErrorEnum.REQUEST_PARAM_ERROR));
        Page<Role> page = new Page<>(reRoleDTO.getPageNum(), reRoleDTO.getPageSize());
        // 这里只根据角色进行模糊匹配
        LambdaQueryWrapper<Role> wrapper = new LambdaQueryWrapper<>();
        wrapper
                .like(!StringUtils.isEmpty(reRoleDTO.getName()), Role::getName, reRoleDTO.getName())
                .eq(Role::getDeleted, StatusEnum.ENTITY_IS_DELETED_NOT_DELETED.getValue());
